...
export async function generateMetadata({
  ...
  const cat = CATEGORY_SEEDS.find((c) => c.slug === slug);
  if (!cat) return {};
  return {
...
export default async function CategoryPage({
  ...
  const products = (PRODUCT_SEEDS[slug] || []).map((p) => ({
    name: p.name,
    slug: p.name
      .toLowerCase()
      .replace(/[–—/]/g, "-")
    ...
    <>
      <JsonLd
        data={breadcrumbSchema([
          ...
          { name: cat.name, path: `/products/${slug}` },
        ])}
      />
      <PageHero
        title={cat.name}
        backgroundImage={
          CATEGORY_IMAGES[slug]?.src ?? "/images/hero-warehouse.jpg"
        }
      />
      <section className="section-pad bg-brand-soft">
        <div className="container-site">
          <Suspense fallback={<div className="py-20 text-center text-sm text-brand-muted">Loading…</div>}>
            <ProductsBrowser products={products} fixedCategorySlug={slug} />
          </Suspense>
        </div>
      </section>
      <CTABanner title={`Need Bulk ${cat.name} Pricing?`} />
    </>
  );
}
